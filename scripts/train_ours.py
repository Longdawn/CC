"""
Ours 训练脚本（λ_ortho=0.01，使用 dual-stream orthogonality）
目的：完整方法，包含 decorrelation loss
"""
import sys
...
def main():
    ...
    print("=" * 60)
    print("OURS TRAINING (λ_ortho = 0.01)")
    print("=" * 60)
    ...
    train_dual_with_lambda_ortho(
        ...
        train_set=train_set,
        device=device,
        exp_dir="experiments/ours",
        epochs=2,  # 和 baseline 一致
        batch_size=4,
        lr=1e-4,
        lambda_bg=0.4,
        lambda_ortho=0.01,  # ← 关键：使用 orthogonality
    )
